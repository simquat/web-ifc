...
namespace webifc
{
    ...
    std::vector<std::pair<double, double>> ComputeRawClipSegments(const std::vector<ClipLine>& sortedClipLines)
    {
        std::vector<std::pair<double, bool>> uniqueClipLines;
        std::stack<ClipLine> clipStack;
        ...
        while (true)
        {
            if (clipStack.empty())
            {
                clipStack.push(sortedClipLines[clipIndex++]);
                hasLine = true;
                uniqueClipLines.emplace_back(clipStack.top().startLineDist, true);
            }

            if (clipIndex == sortedClipLines.size())
            {
                break;
            }

            const auto& clipLine = sortedClipLines[clipIndex];

            if (clipStack.top().HasCornerWithValue(clipLine))
            {
                // ignore clipline
                clipIndex++;
            }
            else if(clipLine.HasCornerWithValue(clipStack.top()))
            {
                // replace
                clipStack.top().startLineDist = clipLine.startLineDist;
                clipStack.top().endLineDist = clipLine.endLineDist;
                clipIndex++;
            }
            else if (clipStack.top().Overlaps2D(clipLine))
            {
                if (clipLine.type == ClipType::ON && hasLine)
                {
                    // for on during line segment, we can assume there's no further interruptions, so process the segment in one go
                    uniqueClipLines.emplace_back(clipLine.startLineDist, true);
                    uniqueClipLines.emplace_back(clipLine.startLineDist, true);
                    uniqueClipLines.emplace_back(clipLine.endLineDist, true);
                }
                else
                {
                    // for above/below, break up the top
                    clipStack.push(clipLine);
                    hasLine = !hasLine;
                    uniqueClipLines.emplace_back(clipStack.top().startLineDist, hasLine);
                }

                clipIndex++;
            }
            /*
            else if (clipLine.Overlaps2D(clipStack.top()))
            {
                if (clipStack.top().type == ClipType::ON && hasLine)
                {
                    // for on during line segment, we can assume there's no further interruptions, so process the segment in one go
                    uniqueClipLines.emplace_back(clipStack.top().endLineDist, true);
                    uniqueClipLines.emplace_back(clipStack.top().endLineDist, true);
                    uniqueClipLines.emplace_back(clipLine.endLineDist, true);
                ...
                {
                    // pop top, try again
                    uniqueClipLines.emplace_back(clipStack.top().endLineDist, !hasLine);
                    hasLine = !hasLine;
                    clipStack.pop();
                }
            }*/
        ...
        for (int i = 0; i < uniqueClipLines.size(); i++)
        {
            double value = uniqueClipLines[i].first;
            if (first.has_value())
            {
                segments.emplace_back(*first, value);
                first = std::nullopt;
            }
            else
            {
    ...
    std::vector<ClipSegment> ComputeClipSegmentsFromRaw(const std::vector<std::pair<double, double>>& rawClipSegments)
    {
        std::vector<ClipSegment> clipSegments;

        for (auto& r : rawClipSegments)
        {
            if (r.first == r.second)
            {
                ClipSegment s;
                ...
                clipSegments.push_back(std::move(s));
            }
            else
            {
                {
        ...
        // remove points which are directly preceded/followed by a line_begin/end on the same spot
        std::vector<ClipSegment> filteredClipSegments;
        for (size_t i = 0; i < clipSegments.size(); i++)
        {
            if (clipSegments[i].type == ClipSegmentType::POINT)
            {
                auto& prev = clipSegments[(i - 1) % clipSegments.size()];
                auto& cur =  clipSegments[(i) % clipSegments.size()];
                auto& next = clipSegments[(i + 1) % clipSegments.size()];

                // ignore epsilon checks here, since these points would be assigned, not computed, the same value
                if (prev.pos != cur.pos && next.pos != cur.pos)
                {
                    filteredClipSegments.push_back(cur);
                }
            }
            else
            {
                filteredClipSegments.push_back(clipSegments[i]);
            }
        }

        return filteredClipSegments;
    }


    std::vector<ClipSegment> LineDistancesToClipSegments2(size_t meshIndex,
                                                         size_t polygonIndex,
                                                         ...
                                                         const glm::dvec3& lineDir)
    {
        ...
        for (int d = 0; d < distances.size(); d++)
        {
            ...
            else if (startOnLine || endOnLine)
            {
                if (startOnLine)
                {
                    // submit crossing point once, if previous was not ON
                    ClipLine ci;
                    ci.originMesh = meshIndex;
                    ci.originPolygon = polygonIndex;
                    ci.type = ClipType::ON;
                ...
                if (startOnLine)
                {
                    ci.type = endSide ? ClipType::ABOVE : ClipType::BELOW;
                }
                else
                {
                    ci.type = startSide ? ClipType::ABOVE : ClipType::BELOW;
                }
                ci.pos = s3D;
                ci.pos2 = e3D;
                clipLines.push_back(ci);
        ...
        if (clipLines.empty())
        {
            // no intersections between the contour and the line
            return {};
        }

        std::vector<ClipLine> filteredClipLines;

        // filter out stretches of above/below so we only get non-overlapping line segments in above/below
        // segments may still overlap when considering ABOVE & BELOW or ABOVE/BELOW & ON
        for (int d = 0; d < clipLines.size(); d++)
        {
            int s = d;
            int e = (d + 1) % clipLines.size();

            const auto& cs = clipLines[s];
            const auto& ce = clipLines[e];

            if (cs.type == ce.type)
            {
                if (cs.type == ClipType::ABOVE || cs.type == ClipType::BELOW)
                {
                    // start & end are above/below, merge cs and ce
                    ClipLine merged = cs;
                    merged.pos2 = ce.pos2;
                    filteredClipLines.push_back(merged);

                    // skip ce in next loop
                    d++;
                }
                else
                {
                    // pass through
                    filteredClipLines.push_back(cs);
                }
            }
            else
            {
        ...
        // we now have "double" lines, each intersection with the shared plane line results in an ABOVE and BELOW Line
        // flatten into line segments by considering only ON + ABOVE, and treating nested ABOVE as holes

        std::vector<ClipLine> sortedClipLines;
        for (auto& cl : filteredClipLines)
        {
            if (cl.type == ClipType::BELOW)
            {
                // skip below
                continue;
            }
            ...
            sortedClipLines.push_back(cl);
        }

        // check the first & last, they may not be properly merged yet
        // TODO: fix this in the previous stage! It's caused by insufficient knowledge when starting the loop
        if (sortedClipLines.size() > 1)
        {
            auto& front = sortedClipLines.front();
            auto& back = sortedClipLines.back();

            if (front.type == back.type && front.type != ClipType::ON)
            {
                sortedClipLines.erase(sortedClipLines.begin());
            }
        }

        std::sort(sortedClipLines.begin(), sortedClipLines.end(), [&](const ClipLine& a, const ClipLine& b)
                  {
                      return a.startLineDist > b.startLineDist;
                  });

        auto rawClipSegments = ComputeRawClipSegments(sortedClipLines);
        auto segs = ComputeClipSegmentsFromRaw(rawClipSegments);
    ...
    std::vector<ClipSegment> LineDistancesToClipSegments2D(const std::vector<glm::dvec2>& points, const std::vector<size_t>& pointIndices, glm::dvec2 linePos2D, glm::dvec2 lineDir2D)
    {
        ...
        return LineDistancesToClipSegments2(0,
                                           0,
                                           distances,
                                           pointIndices,
                                           sourcePoints,
...
//                DumpIfcGeometry(outputGeom, L"output.obj");
    ...
    std::vector<ClipSegment> ClipToLine(const Polygon3D& source, const Polygon3D& target, const glm::dvec3& linePos, const glm::dvec3& lineDir)
    {
        ...
        for (auto& contour : source.contours)
        {
            ...
            auto segs = LineDistancesToClipSegments2(source.meshIndex, 
                                                    source.index, 
                                                    distances, 
                                                    contour, 
                                                    source.sourcePoints,
            ...
            auto backupSegs = LineDistancesToClipSegments(source.meshIndex,
                                                     source.index,
                                                     distances,
                                                     contour,
                                                     source.sourcePoints,
        ...
        bvh1.Intersect(bvh2, [&](uint32_t i, uint32_t j)
                       {
                           ...
                           else
                           {
                               ...
                               {
                                   intersectionLines.push_back(intersect_triangle_triangle_non_coplanar(a, b, c, d, e, f));
                               }
