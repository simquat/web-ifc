...
namespace webifc::geometry
{
  ...
  public:
    ...
    template<size_t N> std::vector<glm::vec<N,double>> GetCartesianPointList(const uint32_t expressID) const;
    ...
    template<size_t N> IfcProfileDef<N> &GetProfile(uint32_t expressID) const;
  ...
  private:
    template<size_t N> IfcProfileDef<N> &GetProfileByLine(uint32_t lineID) const;
    template<size_t N> IfcLoop<N> &GetLoop(const uint32_t expressID) const;
    template<size_t N> glm::vec<N,double> GetVector(uint32_t expressID) const;
    ...
    template<size_t N> glm::vec<N,double> GetVertexPoint(uint32_t expressID) const;
